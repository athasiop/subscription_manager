...
class UserProfilePage(tk.Frame):
    ...
    def __init__(self, parent,cur,user_info,frames):         
        ...
        tk.Button(self, text="Edit", command=self.editName).grid(row=1, column=2)
        
        tk.Label(self,text="Birth Date").grid(row=2)
        tk.Label(self,text="Email = ").grid(row=3)
        tk.Button(self, text="Edit", command=self.editEmail).grid(row=3, column=2)
        tk.Label(self,text="Country = ").grid(row=4)
        tk.Button(self, text="Edit", command=self.editCountry).grid(row=4, column=2)
        tk.Label(self,text="Zip Code = ").grid(row=5)
        tk.Button(self, text="Edit", command=self.editZipCode).grid(row=5, column=2)
        tk.Label(self,text="Street = ").grid(row=6)
        tk.Button(self, text="Edit", command=self.editStreet).grid(row=6, column=2)
        ttk.Button(self,text="Delete Account",command=self.delete_user).grid(row=8,column=1)
